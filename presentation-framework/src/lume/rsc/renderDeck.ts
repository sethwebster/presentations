...
import type { LumePackage, LumeAnimation } from '../types';
import { createLumePackageFromSlides } from '../transform';
import type { DeckDefinition, SlideDefinition, LayerDefinition, ElementDefinition, AnimationDefinition, SlideTransitionType, SlideTransitions } from '../../rsc/types';
...
function mapLumeTransitionToSlideTransition(lumeAnimation: LumeAnimation | undefined): (AnimationDefinition & { type?: SlideTransitionType }) | undefined {
  if (!lumeAnimation) return undefined;
  
  // Map LumeAnimation types to SlideTransitionType
  // Only valid slide transition types are kept, others are ignored
  const transitionMap: Partial<Record<string, SlideTransitionType>> = {
    'fade': 'fade',
    'dissolve': 'dissolve',
    'move-in-left': 'push-left',
    'move-in-right': 'push-right',
    'move-out-left': 'push-left',
    'move-out-right': 'push-right',
    'slide': 'push-left',
    'zoom': 'zoom',
    'zoom-in': 'zoom',
    'zoom-out': 'zoom',
  ...
  const mappedType = transitionMap[lumeAnimation.type];
  if (!mappedType) return undefined;
  
  // Return a full AnimationDefinition
  return {
    id: lumeAnimation.id,
    type: mappedType,
    duration: lumeAnimation.duration,
    delay: lumeAnimation.delay,
    easing: lumeAnimation.easing,
    parameters: lumeAnimation.parameters,
  };
}

function mapLumeTransitions(transitions: { in?: LumeAnimation; out?: LumeAnimation; between?: LumeAnimation } | undefined): SlideTransitions | undefined {
  if (!transitions) return undefined;
  
  const result: SlideTransitions = {};
  if (transitions.in) {
    const mapped = mapLumeTransitionToSlideTransition(transitions.in);
    if (mapped) result.in = mapped;
  }
  if (transitions.out) {
    const mapped = mapLumeTransitionToSlideTransition(transitions.out);
    if (mapped) result.out = mapped;
  }
  if (transitions.between) {
    const mapped = mapLumeTransitionToSlideTransition(transitions.between);
    if (mapped) result.between = mapped;
  }
...
function convertLumePackageToDeckDefinition(pkg: LumePackage): DeckDefinition {
  const slides: SlideDefinition[] = pkg.slides.map((slide, index) => {
    ...
    return {
      ...
      transitions: mapLumeTransitions(slide.transitions),
