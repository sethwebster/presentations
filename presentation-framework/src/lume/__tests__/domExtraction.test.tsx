...
describe('extractElementsFromSlideContent', () => {
  it('returns text element for simple string content', () => {
    const elements = extractElementsFromSlideContent(<h1>Hello Lume</h1>);
  ...
  it('preserves line breaks and nested spans', () => {
    ...
    expect(elements).toHaveLength(1);
    expect(elements[0].content).toBe('How We Build\nWhy We Build');
  });

  it('handles null or empty content safely', () => {
    expect(extractElementsFromSlideContent(null)).toEqual([]);
    expect(extractElementsFromSlideContent(undefined)).toEqual([]);
  });
});
