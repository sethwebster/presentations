  ...
  const handleSend = async () => {
    ...
    try {
      ...
      parser.on('function_call', (event: ConversationStreamEvent) => {
        ...
        if (event.type === 'function_call' && event.data?.arguments) {
          functionCallArgs += JSON.stringify(event.data.arguments);
        }
      });
  ...
  return (
    ...
    >
      ...
      <div className="p-4 border-b border-white/40 dark:border-white/10 flex items-center justify-between">
        <h3 className="text-lg font-semibold" style={{ color: 'var(--lume-mist)' }}>AI Assistant</h3>
        <button
          onClick={onToggle}
      ...
      >
        {messages.length === 0 && (
          <div className="text-center py-8" style={{ color: 'var(--lume-mist)', opacity: 0.7 }}>
