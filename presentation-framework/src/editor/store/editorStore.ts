...
export const useEditorStore = create<EditorStore>((set, get) => ({
  ...
  deselectElement: (elementId: string) => {
    const { selectedElementIds, keyObjectId } = get();
    ...
    set({ selectedElementIds: newSelection, keyObjectId: newKeyObjectId });
  },

  clearSelection: () => {
    set({ selectedElementIds: new Set(), keyObjectId: null });
  },

  setKeyObject: (elementId: string | null) => {
    const { selectedElementIds } = get();
    // Only allow setting key object if it's in the current selection
    if (elementId !== null && !selectedElementIds.has(elementId)) {
      return;
    }
    set({ keyObjectId: elementId });
  },
      ...
      slides: deck.slides.map(slide => ({
        ...
        elements: slide.elements?.map(el => {
          if (el.id === elementId) {
            ...
            let mergedBounds = el.bounds;
            if (updates.bounds) {
              // Ensure we have a base bounds object to merge into
              const baseBounds = el.bounds || { x: 0, y: 0, width: 100, height: 50 };
            ...
            return { ...el, ...updates, bounds: mergedBounds };
          }
          return el;
  ...
  setFocusContext: (context: 'slides' | 'canvas' | null) => {
