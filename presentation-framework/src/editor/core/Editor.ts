...
export interface EditorState {
  ...
  keyObjectId: string | null; // Key object for alignment/distribution
  ...
  focusContext: 'slides' | 'canvas' | null; // Track which area has focus for Cmd+A behavior
  isLoading: boolean;
  error: string | null;
...
export class Editor {
  ...
  setSelectedSlide(slideId: string | null): void {
    ...
    this.setState({ 
      selectedSlideId: slideId,
      selectedElementIds: new Set(),
      keyObjectId: null, // Clear element selection when selecting a slide
    });
  }
  ...
  reorderSlides(fromIndex: number, toIndex: number): void {
    const { deck } = this.state;
    if (!deck || fromIndex < 0 || fromIndex >= deck.slides.length ||
        toIndex < 0 || toIndex >= deck.slides.length) return;
    ...
    this.setState({ deck: updatedDeck, currentSlideIndex: toIndex });
    this.executeCommand({
      type: 'reorderSlides',
  ...
  // Section operations
  createSectionFromSlide(sectionMasterSlideId: string, childSlideId: string): void {
    const { deck } = this.state;
    if (!deck) return;

    const updatedSlides = deck.slides.map(slide => {
      if (slide.id === sectionMasterSlideId) {
        // This becomes the section master - generate a unique section name
        const sectionName = slide.title || `Section ${sectionMasterSlideId.slice(-4)}`;
        return { ...slide, section: sectionName };
      }
      if (slide.id === childSlideId) {
        // This becomes a child of the section
        const sectionName = deck.slides.find(s => s.id === sectionMasterSlideId)?.title || `Section ${sectionMasterSlideId.slice(-4)}`;
        return { ...slide, section: sectionName, sectionIndex: 0 };
      }
      return slide;
    });
    ...
    this.executeCommand({
      type: 'createSection',
      params: { sectionMasterSlideId, childSlideId },
      timestamp: Date.now(),
    });
  }
  ...
  removeSection(sectionName: string): void {
    ...
    const updatedSlides = deck.slides.map(slide => {
      if (slide.section === sectionName) {
        const { section, sectionIndex, ...rest } = slide;
        return rest;
      }
      return slide;
    });
  ...
  deselectElement(elementId: string): void {
    const { selectedElementIds, keyObjectId } = this.state;
    const newSelection = new Set(selectedElementIds);
    newSelection.delete(elementId);
    // Clear key object if it was deselected
    const newKeyObjectId = keyObjectId === elementId ? null : keyObjectId;
    this.setState({ selectedElementIds: newSelection, keyObjectId: newKeyObjectId });
  }

  setKeyObject(elementId: string | null): void {
    const { selectedElementIds } = this.state;
    // Only allow setting key object if it's in the current selection
    if (elementId !== null && !selectedElementIds.has(elementId)) {
      return;
    }
    this.setState({ keyObjectId: elementId });
  }
  ...
  toggleElementVisibility(elementId: string): void {
    ...
    // Find element and check current visibility state
    for (const slide of deck.slides) {
      const allElements = [
        ...(slide.elements || []),
        ...(slide.layers?.flatMap(l => l.elements) || []),
      ...
      if (element) {
        currentElement = element;
        isHidden = (element.metadata as any)?.hidden === true;
        break;
      }
    }
    ...
    const duplicated: ElementDefinition = {
      ...
      bounds: elementToDuplicate.bounds
        ? {
            ...
            width: elementToDuplicate.bounds.width ?? 100,
            height: elementToDuplicate.bounds.height ?? 50,
          }
        : { x: 0, y: 0, width: 100, height: 50 },
    };
  ...
  setFocusContext(context: 'slides' | 'canvas' | null): void {
