...
export function EditorCanvas({ deckId: _deckId }: EditorCanvasProps) {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    // Set focus context to canvas when clicking on canvas
    editor.setFocusContext('canvas');
  ...
  // Log all elements when slide changes
  useEffect(() => {
    if (currentSlide) {
      const allElements = [
      ...
      const elementData = allElements.map((element) => ({
        ...
        fill: (element.style as any)?.fill,
        stroke: (element.style as any)?.stroke,
        isHidden: (element.metadata as any)?.hidden,
        isLocked: (element.metadata as any)?.locked,
      }));
      
      const slideData = {
        slideIndex: currentSlideIndex + 1,
        slideId: currentSlide.id || 'unnamed',
        totalElements: allElements.length,
        elementsFromSlide: currentSlide.elements?.length || 0,
        elementsFromLayers: currentSlide.layers?.reduce((sum, layer) => sum + (layer.elements?.length || 0), 0) || 0,
        ...
        fullSlide: currentSlide,
      };
      
      console.group(`📊 Slide ${currentSlideIndex + 1}: ${currentSlide.id || 'unnamed'}`);
      console.log('JSON dump (copy this):');
    ...
    >
          ...
          {showGrid && (
            <div
              style={{
                ...
                pointerEvents: 'none',
                zIndex: 0, // Grid should be below elements
              }}
            />
          )}
          ...
          {currentSlide && (
            <div 
              ...
              style={{
                ...
                height: '100%',
                position: 'relative',
                zIndex: 1, // Ensure elements container is above background
              }}
            >
              {/* Render elements from slide */}
              {(() => {
                // Collect all elements with their rendering order (including group children)
                const allElements: Array<{ element: ElementDefinition; index: number }> = [];
                let renderIndex = 1; // Start at 1 (0 is for background)
                
                // Helper to recursively add group children
                const addGroupChildren = (group: any) => {
                  if (!group.children) return;
                  group.children.forEach((child: ElementDefinition) => {
                    allElements.push({ element: child, index: renderIndex++ });
                    // Recursively add nested group children
                    if (child.type === 'group') {
                      addGroupChildren(child);
                    }
                  });
                };
                
                // Add elements from slide
                currentSlide.elements?.forEach((element) => {
                  allElements.push({ element, index: renderIndex++ });
                  // Add group children recursively
                  if (element.type === 'group') {
                    addGroupChildren(element);
                  }
                });
                
                // Add elements from layers (sorted by order)
                currentSlide.layers
      ...
      >
        <Button
          variant="ghost"
          size="icon"
          className="text-base text-white transition h-7 w-7 hover:bg-white/10"
