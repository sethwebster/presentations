...
function generateSlideAnimations(slide: SlideDefinition, nextSlide?: SlideDefinition): TimelineDefinition | undefined {
  ...
  const hasTitleOnly = textElements.length === 1 && 
                       ...
                       Number(textElements[0].style.fontSize) > 50;
...
export function recommendAnimations(slide: SlideDefinition): {
  ...
  const isQuote = slide.background?.type === 'gradient' && 
  ...
  if (isQuote) {
    ...
    alternatives.push(
      { type: 'scale', duration: 700 },
      { type: 'magic-move', duration: 1000 }
    );
    ...
    alternatives.push(
      { type: 'enter-up', duration: 600 },
      { type: 'enter-left', duration: 500 }
    );
  } else if (hasImages) {
    recommended.push({
      type: 'fade',
    ...
    alternatives.push(
      { type: 'zoom-in', duration: 700 },
      { type: 'enter-left', duration: 600 }
    );
  } else {
    recommended.push({
      type: 'fade',
