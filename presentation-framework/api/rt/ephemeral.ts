...
- Be generous with paraphrase`,
    ...
    return json(500, { error: String(err?.message ?? error), stack: String(err?.stack ?? '') });
  }
}

// Helper functions
async function safeJson(req: Request): Promise<RequestBody | null> {
  try { return await req.json() as RequestBody; } catch { return null; }
}

function json(status: number, body: SessionResponse | ErrorResponse): Response {
