    ...
    for (const key of keys) {
      ...
      try {
        ...
        if (deckDataJson) {
          ...
          console.log(`[getUserShowcase] Checking deck ${deckId}:`, {
            ownerId: deckData.meta?.ownerId,
            public: deckData.meta?.public,
            userId: userId,
            ownerMatches: deckData.meta?.ownerId === userId,
            isPublic: deckData.meta?.public === true,
          });
          ...
          if (
            ...
            console.log(`[getUserShowcase] Adding public presentation: ${deckData.meta?.title || deckId}`);
        ...
        console.error(`[getUserShowcase] Error loading deck ${deckId}:`, error);
        continue;
      }
    }

    console.log(`[getUserShowcase] Found ${publicPresentations.length} public presentations for user ${username} (ID: ${userId})`);
